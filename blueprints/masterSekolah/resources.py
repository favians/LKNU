...
from .model import MasterSekolah
from blueprints import db, app
from sqlalchemy import desc
# 'client' penamaan (boleh diganti)

bp_masterSekolah = Blueprint('masterSekolah', __name__)
api = Api(bp_masterSekolah)


class MasterSekolahResource(Resource):
    ...
    def get(self):
        ...
        qry = MasterSekolah.query.get(args["id"])

        if qry is not None:
            return {"status":"success", "result":marshal(qry, MasterSekolah.response_field)}, 200, {'Content-Type':'application/json'}
    ...
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('nama_sekolah',location='json', help='invalid nama_sekolah', required=True)
        ...
        qry = MasterSekolah(datetime.datetime.now(), args['nama_sekolah'], args['alamat'], args['kabupaten'],args['kecamatan'],args['desa'],args['no_telepon'],args['email'],args['kontak_person'],claims['id'])
    ...
    def put(self):
        ...
        qry.nama_sekolah = args['nama_sekolah']
...
class MasterSekolahResourceList(Resource):
    ...
    def get(self):
        ...
        for row in qry.limit(args['rp']).offset(offset).all():
            result.append(marshal(row, MasterSekolah.response_field))
...
api.add_resource(MasterSekolahResource, '','')
api.add_resource(MasterSekolahResourceList, '/list')
